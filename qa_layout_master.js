...
const targets=Array.from({length:120},(_,i)=>i+1);
let failed=false;
for(const slide of targets){
 const url=`${harness}?target=${encodeURIComponent('index.html')}&slide=${slide}`;
 ...
 if(obj.outsideCount||slideOverflow||deckOverflow){failed=true;fs.writeFileSync(`qa_master_layout_slide${slide}.json`,JSON.stringify(obj,null,2));}
}
